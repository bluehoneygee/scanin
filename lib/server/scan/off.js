...
export function pickOffImage(p) {
...
export function chooseCategoryFromSignals(sig) {
...
export function enforceConsistency(ai, signals) {
  const cat = chooseCategoryFromSignals(signals);
  if (cat) {
    ai.category = cat;
    ...
    if (!ai.awareness) {
      ...
      ai.awareness = map[cat] || ai.awareness;
    }
  }
